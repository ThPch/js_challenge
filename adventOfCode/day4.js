...
let tirageEx = [7,4,9,5,11,17,23,2,0,14,21,24,10,16,13,6,15,25,12,22,18,20,8,19,3,26,1]

let data = [50,79,88,34,0,
    56,46,5,17,31,
    29,6,38,78,68,
    75,57,15,44,83,
    89,45,43,85,72,
...
const arraySplitter = (size, data) => {
    var resultList = [];

    for (var i=0; i<data.length; i+=size) {
        resultList.push(data.slice(i,i+size));
    }

    return resultList;
}

const bingoList = arraySplitter(25, data)

let bingoMatrix = []

bingoList.forEach((bingo) => {
    let result = arraySplitter(5, bingo)
    bingoMatrix.push(result)
})

/*================================================================================
                            ASSETS FUNCTIONS
================================================================================*/ 
//Permet de savoir si l'array de nombres numbers est contenu dans l'array de nombres lines, si oui, Bingo....
const checker = (numbers, line) => line.every(v => numbers.includes(v))

const sumArray = (arr) => arr.reduce((acc, curr) => acc+curr)

const sumMatrix = (matrix) => sumArray(matrix.flat())

const substractArrays = (bigArray, smallArray) => bigArray.filter(item => !smallArray.includes(item))
...
const columnsToRows = (bingoMatrix) => {
    return bingoMatrix.map((matrix) => getColumns(matrix))
}
...
const isOnLine = (matrix, numbers) => {

    let bingoLine = null;
    matrix.forEach((line,index) => {
        if(checker(numbers, line)) { 
            bingoLine = { indexLine : index, 
                       line : line }
         }
    })
    //On retourne l'index de la ligne dans laquelle se trouve les bons chiffres bingo
    if (bingoLine != null) return bingoLine
}
