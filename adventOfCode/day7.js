...
const countFuel = (crabsList, position) => {
    const newCrabsList = crabsList.map((crab, index) =>{
        if(crab === position) return 0;
        return crab > position ? crab-position : position-crab
    })

    return newCrabsList.reduce((preCrab, nextCrab) => preCrab+nextCrab)
}

const countNewFuel = (crabsList, position) => {
    const newCrabsList = crabsList.map((crab, index) =>{
        if(crab === position) return 0;
        return crab > position ? sumRangeRecursive(crab-position) : sumRangeRecursive(position-crab)
    })

    return newCrabsList.reduce((preCrab, nextCrab) => preCrab+nextCrab)
}

const getCheapestFuel = (crabList, type=null) => {
    const max = Math.max(...data)
    let cheapest = countFuel(data,max);
    if(type === null ) {
        for(let i=0; i<cheapest; i++) {
            cheapest = countFuel(crabList, i) < cheapest ?  countFuel(crabList, i) : cheapest;
        }
    } else {
        for(let i=0; i<cheapest; i++) {
            cheapest = countNewFuel(crabList, i) < cheapest ?  countFuel(crabList, i) : cheapest;
        }
    }
    return cheapest;
} 

// console.log("Result Day 7 Part 1 : " +  getCheapestFuel(data))
console.log("Result Day 7 Part 1 : " +  getCheapestFuel(data,"new"))

//Ne marche pas car l'algo doit être amélioré
console.log("Result Day 7 Part 2 : " +getCheapestFuel(data,"new"))
